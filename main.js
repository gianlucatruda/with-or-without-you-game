...
const sketch = (p) => {
  ...
  function checkCollision() {
    ...
    if (
      p.dist(hedgehog.x, hedgehog.y, balloon.x, balloon.y) <
      (balloon.width + hedgehog.width) / 2 - 3
    ) {
    ...
    if (
      p.dist(hedgehog.x, hedgehog.y, balloon.hoopX, balloon.hoopY) <
      balloon.hoopSize / 2
    ) {
      if (NEWROUND) {
        score++;
  ...
  p.touchStarted = () => {
    if (p.touches.length > 0) {
      // Check if at least one touch is registered
      // console.log(touches);
  ...
  class Balloon {
    ...
    update() {
      ...
      this.hoopY =
        this.y + this.height / 2 + this.stringLen + this.hoopSize / 2;
      this.hoopX = this.x;
    }
    ...
    show() {
      ...
      p.line(
        ...
        this.x + 10,
        this.y + this.height / 2 + this.stringLen,
      ); // Adjust these values to position the string correctly
